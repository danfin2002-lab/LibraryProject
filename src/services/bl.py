...
class BLService:
    ...
    async def get_BL_by_id(self, book_id: int, library_id: int):
    ...
    async def check_BL(self, data: BLAddSchema)->BLSelectSchema:
        existing_bl = await self.repository.check_BL(data)
        return existing_bl
		
    async def add_BL(self, data: BLAddSchema):
        existing_bl = await self.check_BL(data)
        if existing_bl is None:
            await self.repository.add_BL(data)
            return {"ok": "Книга успешно добавлена в библиотеку"}
        else:
            await self.update_BL(data.count, existing_bl)
            return {"ok":"Обновлено количество книг в библиотеке"}
    ...
    async def delete_BL(self, id: int):
        ...
        return {"ok": "Книга из библиотеки успешно удалена"}

    async def update_BL(self, add_count: int, bl_obj: BLSelectSchema)->BLSelectSchema:
