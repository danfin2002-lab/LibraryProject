...
from src.exceptions import ArrearNotFoundException
...
class ArrearService:
    ...
    async def add_arrear(self, data: ArrearAddSchema):
        ...
        if existing_bl.count > 0:
            await self.repository.add_arrear(data)
            await self.bl_service.update_BL(-1, existing_bl)
            return {"ok": "Новая задолженность добавлена"}
        else:
            return {"description": "В библиотеке не хватает таких книг"}
    ...
    async def delete_arrear(self, id: int):
        ...
        return {"ok": "Задолженность успешно удалена"}
